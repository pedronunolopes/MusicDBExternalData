...
@outsystems.hubedition.runtimeplatform.metainformation.EntityRecordDetails(name = "t_MDMusicStyle", key = "wNFAZb5upUOX9dn33pWKkA", ownerKey = "oJe9tylD_EGUBlO4A46FUA", generation = 0, physicalTableName = "[dbo].[t_MDMusicStyle]", dbConnection = "")
public final class ENt_MDMusicStyleEntityRecord implements Serializable, Cloneable, ITypedRecord<ENt_MDMusicStyleEntityRecord>, ISimpleRecord {

	private static final GlobalObjectKey IdMusicStyleId = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*Pb_XsIaDQ02F7i5Jwult_Q");
	private static final GlobalObjectKey IdName = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*LNSQYfU5eUeYR7sA6KTN9A");
	private static final GlobalObjectKey IdDescription = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*50ip7e7ozEuILNfXcRPoxw");
	private static final GlobalObjectKey IdInsertDate = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*sRGARjchkkayFr_mjJ_PXg");
	private static final GlobalObjectKey IdUpdateDate = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*D12evbmdkEeVaP8WunelCg");

	public static void ensureInitialized() {}
	@outsystems.hubedition.runtimeplatform.metainformation.EntityAttributeDetails(name= "MusicStyleId", length=0, isAutonumber=true, isPrimaryKey=true, isEntityReference=false, isMandatory=true)
	private int _ssMusicStyleId;
	public int getSsMusicStyleId() {
		return _ssMusicStyleId;
	}
	public void setSsMusicStyleId(int value) {
		if ((_ssMusicStyleId != value) || OptimizedAttributes.get(0)) {
	...
	@outsystems.hubedition.runtimeplatform.metainformation.EntityAttributeDetails(name= "Description", length=4000, isAutonumber=false, isPrimaryKey=false, isEntityReference=false, isMandatory=false)
	 ...
	 * Default Constructor
	 */
	public ENt_MDMusicStyleEntityRecord() {
		ChangedAttributes = BitSetUtils.newInstance(5, true);
	 ...
	 */
	public RCt_MDMusicStyleRecord toRecord() {
		try {
			return new RCt_MDMusicStyleRecord(this);
		} catch (Exception e) {
	...
	public int read(OSResultSet r, int index) throws ClassCastException {
		setSsMusicStyleId(DataReaderUtils.readInteger(r, index++, "t_MDMusicStyle.MusicStyleId", 0));
		setSsName(DataReaderUtils.readText(r, index++, "t_MDMusicStyle.Name", ""));
		setSsDescription(DataReaderUtils.readText(r, index++, "t_MDMusicStyle.Description", ""));
		setSsInsertDate(DataReaderUtils.readDateTime(r, index++, "t_MDMusicStyle.InsertDate", new DateTime(1900, 1, 1, 0, 0, 0)));
		setSsUpdateDate(DataReaderUtils.readDateTime(r, index++, "t_MDMusicStyle.UpdateDate", new DateTime(1900, 1, 1, 0, 0, 0)));
	 ...
	 *         Record
	 */
	public void readIM(ENt_MDMusicStyleEntityRecord r) throws Exception {
	...
	public boolean equals(Object obj) {
		if ((obj == null) || !(obj instanceof ENt_MDMusicStyleEntityRecord)) {
			return false;
		}
		ENt_MDMusicStyleEntityRecord b = (ENt_MDMusicStyleEntityRecord) obj;
	 ...
	 */
	public ENt_MDMusicStyleEntityRecord duplicate() throws Exception {
		ENt_MDMusicStyleEntityRecord t = new ENt_MDMusicStyleEntityRecord();
	...
	public void toXml(Object parent, Element baseElem, String fieldName, int detailLevel) {
		...
		if (detailLevel > 0) {
			if (!VarValue.fieldIsOptimized(parent, fieldName + ".MusicStyleId")) VarValue.appendAttribute(recordElem, "MusicStyleId", Integer.valueOf(getSsMusicStyleId()), detailLevel, TypeKind.Integer); else VarValue.appendOptimizedAttribute(recordElem, "MusicStyleId");
	...
	public Object clone() throws CloneNotSupportedException {
		ENt_MDMusicStyleEntityRecord t = (ENt_MDMusicStyleEntityRecord) super.clone();
		return t;
	}
	...
	public void fillFromOther(IRecord other) throws Exception {
		...
		ChangedAttributes.set(0, other.changedAttributeGet(IdMusicStyleId));
		OptimizedAttributes.set(0, other.optimizedAttributeGet(IdMusicStyleId));
