...
@outsystems.hubedition.runtimeplatform.metainformation.EntityRecordDetails(name = "t_MDMedia", key = "nZRqQtf0A0uILEqoSmuzww", ownerKey = "oJe9tylD_EGUBlO4A46FUA", generation = 0, physicalTableName = "[dbo].[t_MDMedia]", dbConnection = "")
public final class ENt_MDMediaEntityRecord implements Serializable, Cloneable, ITypedRecord<ENt_MDMediaEntityRecord>, ISimpleRecord {

	private static final GlobalObjectKey IdMediaTypeId = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*_ec16wVN1ES4RSBlKy3N9w");
	private static final GlobalObjectKey IdName = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*IIU+SMgRT0SJkm9abiOW3Q");
	private static final GlobalObjectKey IdDescription = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*L1e6h+Ez3kWL1qP12KoqPA");
	private static final GlobalObjectKey IdInsertDate = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*C1paOf8ZUE26XlFSy5Xm3g");
	private static final GlobalObjectKey IdUpdateDate = GlobalObjectKey.parse("oJe9tylD_EGUBlO4A46FUA*0B32KOSVx0GMhCUZhvaOjg");

	public static void ensureInitialized() {}
	@outsystems.hubedition.runtimeplatform.metainformation.EntityAttributeDetails(name= "MediaTypeId", length=0, isAutonumber=true, isPrimaryKey=true, isEntityReference=false, isMandatory=true)
	private int _ssMediaTypeId;
	public int getSsMediaTypeId() {
		return _ssMediaTypeId;
	}
	public void setSsMediaTypeId(int value) {
		if ((_ssMediaTypeId != value) || OptimizedAttributes.get(0)) {
	 ...
	 * Default Constructor
	 */
	public ENt_MDMediaEntityRecord() {
		ChangedAttributes = BitSetUtils.newInstance(5, true);
	 ...
	 */
	public RCt_MDMediaRecord toRecord() {
		try {
			return new RCt_MDMediaRecord(this);
		} catch (Exception e) {
	...
	public int read(OSResultSet r, int index) throws ClassCastException {
		setSsMediaTypeId(DataReaderUtils.readInteger(r, index++, "t_MDMedia.MediaTypeId", 0));
		setSsName(DataReaderUtils.readText(r, index++, "t_MDMedia.Name", ""));
		setSsDescription(DataReaderUtils.readText(r, index++, "t_MDMedia.Description", ""));
		setSsInsertDate(DataReaderUtils.readDateTime(r, index++, "t_MDMedia.InsertDate", new DateTime(1900, 1, 1, 0, 0, 0)));
		setSsUpdateDate(DataReaderUtils.readDateTime(r, index++, "t_MDMedia.UpdateDate", new DateTime(1900, 1, 1, 0, 0, 0)));
	 ...
	 *         Record
	 */
	public void readIM(ENt_MDMediaEntityRecord r) throws Exception {
	...
	public boolean equals(Object obj) {
		if ((obj == null) || !(obj instanceof ENt_MDMediaEntityRecord)) {
			return false;
		}
		ENt_MDMediaEntityRecord b = (ENt_MDMediaEntityRecord) obj;
	 ...
	 */
	public ENt_MDMediaEntityRecord duplicate() throws Exception {
		ENt_MDMediaEntityRecord t = new ENt_MDMediaEntityRecord();
	...
	public void toXml(Object parent, Element baseElem, String fieldName, int detailLevel) {
		...
		if (detailLevel > 0) {
			if (!VarValue.fieldIsOptimized(parent, fieldName + ".MediaTypeId")) VarValue.appendAttribute(recordElem, "MediaTypeId", Integer.valueOf(getSsMediaTypeId()), detailLevel, TypeKind.Integer); else VarValue.appendOptimizedAttribute(recordElem, "MediaTypeId");
	...
	public Object clone() throws CloneNotSupportedException {
		ENt_MDMediaEntityRecord t = (ENt_MDMediaEntityRecord) super.clone();
		return t;
	}
	...
	public void fillFromOther(IRecord other) throws Exception {
		...
		ChangedAttributes.set(0, other.changedAttributeGet(IdMediaTypeId));
		OptimizedAttributes.set(0, other.optimizedAttributeGet(IdMediaTypeId));
